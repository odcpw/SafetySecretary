-- Photo evidence captions: user descriptions/explanations per attachment.
-- Follows the idempotent apply-function pattern of 00310.

CREATE OR REPLACE FUNCTION "shared"."apply_incident_attachment_caption_schema"(tenant_schema name)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  WHERE namespace.nspname = tenant_schema::text
    AND class.relname = 'incident_attachment'
    AND class.relkind = 'r'
  INTO attachment_oid;

  IF attachment_oid IS NULL THEN
    RAISE EXCEPTION 'incident_attachment must exist before attachment captions in schema: %', tenant_schema
      USING ERRCODE = '42P01';
  END IF;

  EXECUTE format(
    'ALTER TABLE %I.incident_attachment ADD COLUMN IF NOT EXISTS caption text',
    tenant_schema
  );
END;
$$;

CREATE OR REPLACE FUNCTION "shared"."apply_incident_attachment_caption_schema_to_all_tenants"()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
SELECT "shared"."apply_incident_attachment_caption_schema_to_all_tenants"();

-- Down migration reference for manual rollback in development:
-- 1. ALTER TABLE <tenant>.incident_attachment DROP COLUMN caption;
-- 2. DROP FUNCTION shared.apply_incident_attachment_caption_schema_to_all_tenants();
-- 3. DROP FUNCTION shared.apply_incident_attachment_caption_schema(name);
-- 4. Re-apply db/sql/00310_incident_cause_branch_status.sql to restore the
