-- Soft delete for incident cases: a recoverable deleted_at timestamp.
-- A safety record must be recoverable, and a hard delete would fight the
-- cause/action/coach-message/snapshot foreign keys, so the register hides
-- cases where deleted_at IS NOT NULL instead of removing the row.
-- Follows the idempotent apply-function pattern of 00330.

CREATE OR REPLACE FUNCTION "shared"."apply_incident_soft_delete_schema"(tenant_schema name)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF incident_case_oid IS NULL THEN
    RAISE EXCEPTION 'incident_case must exist before soft delete in schema: %', tenant_schema
      USING ERRCODE = '42P01';
  END IF;

  EXECUTE format(
    'ALTER TABLE %I.incident_case ADD COLUMN IF NOT EXISTS deleted_at timestamptz',
    tenant_schema
  );
END;
$$;

CREATE OR REPLACE FUNCTION "shared"."apply_incident_soft_delete_schema_to_all_tenants"()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
SELECT "shared"."apply_incident_soft_delete_schema_to_all_tenants"();

-- Down migration reference for manual rollback in development:
-- 1. ALTER TABLE <tenant>.incident_case DROP COLUMN deleted_at;
-- 2. DROP FUNCTION shared.apply_incident_soft_delete_schema_to_all_tenants();
-- 3. DROP FUNCTION shared.apply_incident_soft_delete_schema(name);
-- 4. Re-apply db/sql/00330_incident_workflow_stage.sql to restore the
