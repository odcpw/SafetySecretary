-- Incident date/time must not be fabricated at creation. A blank chat-first
-- draft has no known incident_at yet; the coach sets it once the conversation
-- reveals it. Storing now() (the moment "New" was clicked) records a fact that
-- is not true, so the column must be nullable until that fact exists.
--
-- The base table in 00200 already declares incident_at as a nullable
-- timestamptz, but a live tenant may have had a NOT NULL constraint applied at
-- some point. This migration idempotently drops NOT NULL across every tenant
-- schema and re-anchors provision_tenant_schema so new tenants inherit the
-- nullable column. Follows the idempotent apply-function pattern of 00340.

CREATE OR REPLACE FUNCTION "shared"."apply_incident_at_nullable_schema"(tenant_schema name)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF incident_case_oid IS NULL THEN
    RAISE EXCEPTION 'incident_case must exist before relaxing incident_at in schema: %', tenant_schema
      USING ERRCODE = '42P01';
  END IF;

  EXECUTE format(
    'ALTER TABLE %I.incident_case ALTER COLUMN incident_at DROP NOT NULL',
    tenant_schema
  );
END;
$$;

CREATE OR REPLACE FUNCTION "shared"."apply_incident_at_nullable_schema_to_all_tenants"()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
SELECT "shared"."apply_incident_at_nullable_schema_to_all_tenants"();

-- Down migration reference for manual rollback in development:
-- 1. Backfill any rows that must keep a date, then:
--    ALTER TABLE <tenant>.incident_case ALTER COLUMN incident_at SET NOT NULL;
-- 2. DROP FUNCTION shared.apply_incident_at_nullable_schema_to_all_tenants();
-- 3. DROP FUNCTION shared.apply_incident_at_nullable_schema(name);
-- 4. Re-apply db/sql/00340_incident_soft_delete.sql to restore the
