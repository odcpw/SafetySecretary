-- Process Mapping quest-loop field for every provisioned tenant schema.
-- who_would_know stores the next first-person narrator for fog/haze blocks.

CREATE OR REPLACE FUNCTION "shared"."apply_process_map_quest_schema"(tenant_schema name)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  EXECUTE format(
    'ALTER TABLE %I.process_node ADD COLUMN IF NOT EXISTS who_would_know text',
    tenant_schema
  );
END
$$;

CREATE OR REPLACE FUNCTION "shared"."apply_process_map_quest_schema_to_all_tenants"()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  FOR tenant_schema IN
    ...
    ORDER BY nspname
  LOOP
    PERFORM "shared"."apply_process_map_quest_schema"(tenant_schema);
  END LOOP;
END
$$;
...
SELECT "shared"."apply_process_map_quest_schema_to_all_tenants"();

-- Down migration reference for manual rollback in development:
-- 1. ALTER TABLE <tenant>.process_node DROP COLUMN who_would_know;
-- 2. DROP FUNCTION shared.apply_process_map_quest_schema_to_all_tenants();
-- 3. DROP FUNCTION shared.apply_process_map_quest_schema(name);
-- 4. Re-apply db/sql/00470_process_map_edges.sql to restore the previous
--    provision layer.
