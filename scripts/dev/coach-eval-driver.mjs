...
//
// Usage (on VPS): set -a; . /opt/safetysecretary/.env; set +a
...
const METHODS = (
	process.env.EVAL_METHODS ?? "FIVE_WHYS,URSACHENBAUM,ISHIKAWA"
).split(",");
...
function baseHeaders(stateChanging) {
	...
	if (stateChanging && CSRF) h["x-safetysecretary-csrf"] = CSRF;
	return h;
}

async function login() {
	const r = await fetch(`${BASE}/api/auth/dev-session`, {
		method: "POST",
	...
	COOKIE = Object.entries(jar)
		.map(([k, v]) => `${k}=${v}`)
		.join("; ");
	CSRF = jar.safetysecretary_csrf ?? jar.ssfw_csrf ?? "";
	if (!(jar.safetysecretary_session ?? jar.ssfw_session) || !CSRF)
		throw new Error(
			`login failed (${r.status}): cookies=${Object.keys(jar).join(",")}`,
		);
}
...
async function reporterReply(history) {
	...
	for (const h of history) {
		messages.push({
			role: h.role === "reporter" ? "assistant" : "user",
			content: h.content,
		});
	}
	...
	if (!r.ok)
		throw new Error(
			`reporter LLM ${r.status}: ${JSON.stringify(j).slice(0, 300)}`,
		);
...
async function runMethod(method) {
	...
	const incidentId =
		draft.json?.incident?.id ?? draft.json?.id ?? draft.json?.incidentId;
	if (!incidentId)
		throw new Error(
			`no incident id from draft: ${draft.status} ${draft.text.slice(0, 200)}`,
		);
	// 2. set method
	const setm = await jpost(`/api/incidents/${incidentId}/cause-method`, {
		causeMethod: method,
	});
	// 3. converse
	const history = [];
	const transcript = [];
	let reporterMsg =
		"Bei uns ist gestern an der Fräsmaschine was passiert. Ein Mitarbeiter wollte die Späne mit dem Metallmassstab wegmachen während die Maschine noch lief, und dabei hat es ihm den Finger in den Fräser gezogen. Der Finger ist jetzt teilweise ab.";
	let closedSeen = 0;
	for (let turn = 0; turn < MAX_TURNS; turn++) {
		transcript.push({ role: "reporter", content: reporterMsg });
		history.push({ role: "reporter", content: reporterMsg });
		const chat = await jpost(`/api/incidents/${incidentId}/coach/chat`, {
			locale: "de",
			message: reporterMsg,
		});
		if (chat.status !== 200) {
			transcript.push({
				role: "error",
				content: `chat ${chat.status}: ${chat.text.slice(0, 200)}`,
			});
			break;
		}
		// fetch canonical messages (ops carry ids)
		const msgs =
			(await jget(`/api/incidents/${incidentId}/coach/chat`)).json?.messages ??
			[];
		const lastAssist = [...msgs]
			.reverse()
			.find((m) => m.role === "assistant" || m.role === "coach");
		const coachContent =
			lastAssist?.content ??
			chat.json?.assistantMessage?.content ??
			"(no content)";
		...
		for (const op of ops) {
			...
			const res = await jpost(`/api/incidents/${incidentId}/coach/chat/apply`, {
				action: "apply",
				messageId: lastAssist.id,
				operationId: op.id,
				operationRecordMap: recordMap,
			});
			if (res.json?.ok && res.json?.applied?.recordId && op.ref)
				recordMap[op.ref] = res.json.applied.recordId;
			applied.push({
				kind: op.kind,
				ref: op.ref ?? null,
				ok: !!res.json?.ok,
		...
		if (looksLikeClose(coachContent)) {
			closedSeen++;
			if (closedSeen >= 1) {
				reporterMsg = "Ja, danke, das passt so.";
				if (closedSeen >= 2) break;
			}
		} else {
			reporterMsg = await reporterReply(history);
		}
	}
...
for (const m of METHODS) {
	...
	try {
		out[m] = await runMethod(m.trim());
	} catch (e) {
		out[m] = { method: m, error: String(e) };
		process.stderr.write(`ERROR ${m}: ${e}\n`);
	}
}
...
for (const m of METHODS) {
	const r = out[m.trim()];
	process.stderr.write(
		`${m}: ${r?.error ? "ERROR " + r.error : r?.incidentId + " turns=" + r?.transcript?.length}\n`,
	);
}
