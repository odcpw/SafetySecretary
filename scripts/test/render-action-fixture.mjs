...
const compiledRoot =
	...
	join(process.cwd(), ".tmp", "ssfw-action-fixture-test");
...
const [{ ActionBoardFixture }, { ACTION_BOARD_FIXTURES }] = await Promise.all([
	import(`file://${join(compiledRoot, "components/actions/__fixtures__/action-board.js")}`),
...
assertIncludes(html, "Aktionsbeschreibung", "German description field label should render");
assertIncludes(html, "Status", "German status field label should render");
assertIncludes(html, "Anhaenge", "German attachments field label should render");
assertIncludes(html, "Ueberfaellige Aktionen", "German overdue metric should render");
...
assertIncludes(html, "Keine passenden Aktionen", "German no-matches state should render");
assertIncludes(html, "Anhang hinzufuegen", "German add-attachment command should render");
assertIncludes(html, "Als erledigt markieren", "German action command should render");
assertIncludes(html, "Aktion schliessen", "German close-action command should render");
assertExcludes(html, first.title.en, "English fixture title must not render in DE fixture");

console.log(JSON.stringify({ renderedLocale: "de", fixtureCount: ACTION_BOARD_FIXTURES.length }));
