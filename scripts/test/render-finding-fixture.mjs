...
const compiledRoot =
	process.env.SAFETYSECRETARY_FINDINGS_FIXTURE_BUILD_DIR ??
	process.env.SSFW_FINDINGS_FIXTURE_BUILD_DIR ??
	join(process.cwd(), ".tmp", "safetysecretary-findings-fixture-test");
...
const [{ FindingListFixture }, { FINDING_FIXTURES }] = await Promise.all([
	import(
		`file://${join(compiledRoot, "components/findings/__fixtures__/finding-list.js")}`
	),
...
assertIncludes(
	html,
	"Gute Beobachtung",
	"German good-catch badge should render",
);
assertIncludes(
	html,
	"keine Schuldzuweisung",
	"German no-blame note should render",
);
assertExcludes(
	html,
	first.title.en,
...
console.log(
	JSON.stringify({
		renderedLocale: "de",
		fixtureCount: FINDING_FIXTURES.length,
	}),
);
