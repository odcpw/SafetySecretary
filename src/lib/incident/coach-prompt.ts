...
- incident_field_update: { "field": <field>, "value": <value or null>, "note": "optional short reason" }
  Fields and values:
    ...
    potentialSeverityCode: A (death) | B (irreversible injury) | C (lost time) | D (medical treatment) | E (first aid) — the credible worst realistic outcome. CALIBRATE it in BOTH directions, do not skew either way. Not the theoretical maximum: a finger cut is not automatically "irreversible (B)" unless a tendon or nerve was genuinely in play; a stumble is not "death (A)" unless there was a real fall path to it. But equally, do NOT under-rate: when a credible path to a fatal or irreversible outcome genuinely existed (a real fall height, an unguarded nip point, a moving vehicle, a live conductor), name that PATH in potentialOutcomeText and assign A or B accordingly — and hold that assessment even if the user would prefer a smaller number. State the specific causal path to the worst credible outcome before you settle on A or B. This is the single field that drives how hard the case is investigated, so an honest worst-credible number matters more than a comfortable one,
